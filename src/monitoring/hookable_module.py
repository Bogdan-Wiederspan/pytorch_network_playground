...
class HookableMixin:
    ...
    def monitor_gradient(self, tensor, name):
        ...
        if not tensor.requires_grad:
            warnings.warn(f"Tensor {name} does not requires grad: Skipping")
            return
        ...
        def _safe_callback(grad):
            try:
                ...
                warnings.warn(f"Gradient callback {name} raised {e}")
    ...
    def monitor_tensor(self, tensor, name):
        ...
        try:
            ...
            warnings.warn(f"Tensor callback {name} raised {e}")
        callback(tensor)
